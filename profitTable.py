...
class ProfitTable():
    ...
    def get_all_data(self, start = '2021-08-01', end = date.today()):
        ...
        return {'df': df, 'tprice': tprice, 'tpackage_num': tpackage_num, 'tb_price':tb_price, 'tc_price':tc_price, 'profit':profit }
            ...
            tem.append({'name': name, 'num':num, 'day': row['day'], 'price_average': row['price_average'], 'package_num_average':row['package_num_average'], 'tb_price': buy_price*float(num), 'tc_price' :sell_price*float(num), 'profit':  sell_price*float(num)-buy_price*float(num)})
